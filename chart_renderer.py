            ...
            if getattr(self, "show_karakamsha", True) and is_d1_chart and getattr(self, "chart_data", None) and not getattr(self, "use_circular", False):
                ...
                if ak_planet and "deg_in_sign" in ak_planet and "sign_index" in ak_planet:
                    ...
                    if h_data and k_house in self.house_polys:
                        ...
                        if not house_tints:
                            glow_color = QColor(255, 215, 0) # Clean canvas: Blazing Gold
                        else:
                        ...
                        # --- 2. PIXMAP & MATH CACHING (The Ultimate CPU Saver) ---
                        # We only rebuild the heavy vector glow if the window resizes or colors change
                        cache_key = (k_house, w, h, glow_color.rgb(), getattr(self, "outline_mode", ""))
                        if getattr(self, '_k_cache_key', None) != cache_key:
                            self._k_cache_key = cache_key
                            
                            # Cache A: Pre-calculate the particle math seeds (saves 48 trig calls per frame)
                            self._k_seeds = [(abs(math.sin(i * 37.1)), abs(math.cos(i * 91.3))) for i in range(24)]
                            
                            # Cache B: Pre-render the heavy vector bloom to a transparent image
                            self._k_glow_pix = QPixmap(self.size())
                            ...
                            # Draw the heavy multi-layered glow ONCE
                        ...
                        # --- 3. ANIMATION RENDER LOOP (Now lightning fast) ---
                        current_time = time.time()
                        breath = (math.sin(current_time * 2.0) + 1.0) / 2.0
                        
                        painter.save()
                        
                        # Clip path for the internal magic window
                        clip_path = QPainterPath()
                        clip_path.addPolygon(poly)
                        painter.setClipPath(clip_path)
                        
                        # A. Ambient Aura (Very fast gradient)
                        ...
                        # B. Soul Wind Stardust (Using cached seeds)
                        ...
                        # C. Draw the Pre-Rendered Glow Image (Instantly blitted with pulsing opacity)
                        painter.save()
                        # Pulse the entire image opacity between 60% and 100%

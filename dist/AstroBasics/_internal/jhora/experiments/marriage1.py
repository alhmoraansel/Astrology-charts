...
DT_PERFECT_ORB              = 2.0
DT_PERFECT_PAIR_WINDOW_DAYS = 15      # if you ever enable strict booster
...
DT_STRONG_AWARD      = 5              # score awarded when fallback hits (was 3)
    ...
    for year in range(iter_start_year, iter_end_year + 1):
        ...
        for month in range(1, 13):
            ...
            dtp_score, dtp_detail = _score_dt_perfect_in_month(
                jd, place, month_start, month_end, tol=2.0, sev_target='either', pair_window_days=15
            )
            ...
            dt_strong_score, dt_strong_detail = _score_dt_strong_sign_in_month(
                ...
                jup_target='either', jup_mode='conj', tol=2.0
            )
            ...
            # If nothing fires at all (no vmd & no DT/Part-3), skip early
            if vmd == 0 and (dtp_score == 0 and p3_score == 0 and dt_strong_score == 0):
                continue
            ...
            total = (vmd + jtransit + scd_score_year + tajaka_score +
                     dtp_score + p3_score + dt_strong_score +
            ...
            if total > 0:
                ...
                reasons['dt_strong_score']   = dt_strong_score
...
def _get_varshaphala_marriage_triggers(jd, place, year, derived):
    ...
    patyayini_raw = patyayini.patyayini_dhasa(jd_vf_local, place, divisional_chart_factor=1, chart_method=1)

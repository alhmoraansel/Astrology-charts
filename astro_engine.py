...
class EphemerisEngine:
    ...
    def get_div_sign_and_lon(self, lon_deg, div_type):
        ...
        """
        # Convert to milli-arc-seconds for precise integer division avoiding float drift
        ...
        if div_type in self.custom_vargas:
            ...
            new_sign_idx = (starts[sign_index] + segment) % 12
            deg_remainder = (deg_in_sign % (30.0 / parts)) * parts
            return new_sign_idx, (new_sign_idx * 30.0) + deg_remainder
        ...
        if div_type == "D1": 
            ...
            # Hora
            if sign_index % 2 == 0:
                new_sign_idx = 4 if deg_in_sign < 15.0 else 3
            else:
                new_sign_idx = 3 if deg_in_sign < 15.0 else 4
            deg_rem = (deg_in_sign % 15.0) * 2.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D3":
            # Drekkana
            new_sign_idx = (sign_index + int(deg_in_sign / 10.0) * 4) % 12
            deg_rem = (deg_in_sign % 10.0) * 3.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D4":
            # Chaturthamsha
            new_sign_idx = (sign_index + int(deg_in_sign / 7.5) * 3) % 12
            deg_rem = (deg_in_sign % 7.5) * 4.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D5":
            # Panchamsha
            new_sign_idx = (sign_index + int(deg_in_sign / 6.0)) % 12
            deg_rem = (deg_in_sign % 6.0) * 5.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D6":
            # Shashthamsha
            new_sign_idx = (sign_index + int(deg_in_sign / 5.0)) % 12
            deg_rem = (deg_in_sign % 5.0) * 6.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D7":
            # Saptamsha
            start_sign = (sign_index + 6) % 12 if sign_index % 2 != 0 else sign_index
            new_sign_idx = (start_sign + int(deg_in_sign / (30.0 / 7.0))) % 12
            deg_rem = (deg_in_sign % (30.0 / 7.0)) * 7.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D8":
            # Ashtamsha
            new_sign_idx = (sign_index + int(deg_in_sign / 3.75)) % 12
            deg_rem = (deg_in_sign % 3.75) * 8.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D9":
            # Navamsha
            part = mas_in_sign // 12000000
            start_sign = [0, 9, 6, 3][sign_index % 4]
            new_sign_idx = (start_sign + part) % 12
            deg_rem = ((mas_in_sign % 12000000) / 12000000.0) * 30.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D10":
            # Dashamsha
            part = mas_in_sign // 10800000
            start_sign = sign_index if sign_index % 2 == 0 else sign_index + 8
            new_sign_idx = (start_sign + part) % 12
            deg_rem = ((mas_in_sign % 10800000) / 10800000.0) * 30.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D11":
            # Rudramsha
            new_sign_idx = (sign_index + int(deg_in_sign / (30.0/11.0))) % 12
            deg_rem = (deg_in_sign % (30.0/11.0)) * 11.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D12":
            # Dwadashamsha
            new_sign_idx = (sign_index + int(deg_in_sign / 2.5)) % 12
            deg_rem = (deg_in_sign % 2.5) * 12.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D16":
            # Shodashamsha
            start_sign = [0, 4, 8][sign_index % 3]
            new_sign_idx = (start_sign + int(deg_in_sign / 1.875)) % 12
            deg_rem = (deg_in_sign % 1.875) * 16.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D20":
            # Vimshamsha
            part = mas_in_sign // 5400000
            start_sign = [0, 8, 4][sign_index % 3]
            new_sign_idx = (start_sign + part) % 12
            deg_rem = ((mas_in_sign % 5400000) / 5400000.0) * 30.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D24":
            # Chaturvimshamsha
            start_sign = 3 if sign_index % 2 != 0 else 4
            new_sign_idx = (start_sign + int(deg_in_sign / 1.25)) % 12
            deg_rem = (deg_in_sign % 1.25) * 24.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D27":
            # Bhamsha
            start_sign = [0, 3, 6, 9][sign_index % 4]
            new_sign_idx = (start_sign + int(deg_in_sign / (30.0/27.0))) % 12
            deg_rem = (deg_in_sign % (30.0/27.0)) * 27.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D30":
            # Trimshamsha (Uneven divisions)
            if sign_index % 2 == 0: 
                if mas_in_sign < 18000000: # First 5 degrees
                    new_sign = 0
                    deg = (mas_in_sign / 18000000.0) * 30.0
                elif mas_in_sign < 36000000: # Next 5 degrees
                    new_sign = 10
                    deg = ((mas_in_sign - 18000000) / 18000000.0) * 30.0
                elif mas_in_sign < 64800000: # Next 8 degrees
                    new_sign = 8
                    deg = ((mas_in_sign - 36000000) / 28800000.0) * 30.0
                elif mas_in_sign < 90000000: # Next 7 degrees
                    new_sign = 2
                    deg = ((mas_in_sign - 64800000) / 25200000.0) * 30.0
                else: # Final 5 degrees
                    new_sign = 6
                    deg = ((mas_in_sign - 90000000) / 18000000.0) * 30.0
            else:
                if mas_in_sign < 18000000: 
                    new_sign = 1
                    deg = (mas_in_sign / 18000000.0) * 30.0
                elif mas_in_sign < 43200000: 
                    new_sign = 5
                    deg = ((mas_in_sign - 18000000) / 25200000.0) * 30.0
                elif mas_in_sign < 72000000: 
                    new_sign = 11
                    deg = ((mas_in_sign - 43200000) / 28800000.0) * 30.0
                elif mas_in_sign < 90000000: 
                    new_sign = 9
                    deg = ((mas_in_sign - 72000000) / 18000000.0) * 30.0
                else: 
                    new_sign = 7
                    deg = ((mas_in_sign - 90000000) / 18000000.0) * 30.0
                    
            return new_sign, (new_sign * 30.0) + deg
            
        elif div_type == "D40":
            # Khavedamsha
            start_sign = 0 if sign_index % 2 == 0 else 6
            new_sign_idx = (start_sign + int(deg_in_sign / 0.75)) % 12
            deg_rem = (deg_in_sign % 0.75) * 40.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D45":
            # Akshavedamsha
            start_sign = [0, 4, 8][sign_index % 3]
            new_sign_idx = (start_sign + int(deg_in_sign / (30.0/45.0))) % 12
            deg_rem = (deg_in_sign % (30.0/45.0)) * 45.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
            
        elif div_type == "D60":
            # Shashtiamsha
            part = mas_in_sign // 1800000
            new_sign_idx = (sign_index + part) % 12
            deg_rem = ((mas_in_sign % 1800000) / 1800000.0) * 30.0
            return new_sign_idx, (new_sign_idx * 30.0) + deg_rem
